...
src_install() {
	...
	kmake INSTALL_MOD_PATH="${D}/usr/lib/debug/usr" \
		  INSTALL_FW_PATH="${T}/fw" \
		  modules_install
#4.14.48-coreos-r2-rt30
#	rm "${D}/usr/lib/debug/usr/lib/modules/${KV_FULL}/"modules.* || die
#	rm "${D}/usr/lib/debug/usr/lib/modules/${KV_FULL}/"{source,build} || die
	rm "${D}/usr/lib/debug/usr/lib/modules/4.14.48-coreos-r2/"modules.* || die 
	rm "${D}/usr/lib/debug/usr/lib/modules/4.14.48-coreos-r2/"{source,build} || die
