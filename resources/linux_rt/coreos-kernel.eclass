...
install_build_source() {
	echo "coreos-kernel install_build_source"
	echo "tc_arch_kernel is " ${tc-arch-kernel}
	echo "kv_full is " ${KV_FULL}
	echo "var D is " ${D}
	local kernel_arch=$(tc-arch-kernel)
...
coreos-kernel_pkg_setup() {
	...
	echo "coreos-kernel in coreos-kernel_pkg_setup"
	...
	OUTPUT_DIR="/lib/modules/${KV_MAJOR}.${KV_MINOR}.${KV_PATCH}${KV_EXTRA}-rt30/build"
	KV_FULL=${KV_FULL}-rt30
	...
	echo "coreos-kernel_pkg_setup kv_out_dir " ${KV_OUT_DIR}
	echo "coreos-kernel_pkg_setup var D " ${D}
}

coreos-kernel_src_unpack() {
	echo "coreos-kernel_src_unpack at " `pwd`
	echo "VAR S at " ${S}
	echo "VAR kernel dir at " ${KERNEL_DIR}
...
coreos-kernel_src_configure() {
	echo "coreos-kernel_src_configure at " `pwd`
